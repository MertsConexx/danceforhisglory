...
export async function notifyWhatsApp(env, text) {
  const provider = (env.WA_PROVIDER || "none").toLowerCase();
  if (provider === "none" || provider === "off" || !provider) {
    return { sent: false, reason: "no WhatsApp provider configured" };
  }
...
/**
 * Email notification via Resend. Skipped if RESEND_API_KEY or EMAIL_TO is unset.
 *
 * The default sender is Resend's shared test address, which can only deliver to
 * the address on your own Resend account. That is exactly what is wanted here —
 * notifications to yourself — and it needs no domain verification. To send from
 * an address on danceforhisglory.co.za instead, verify the domain in Resend and
 * set EMAIL_FROM accordingly.
 *
 * Never throws: a failed notification must not cost us the donor.
 */
export async function notifyEmail(env, subject, text, replyTo) {
  ...
  try {
    const body = {
      from: env.EMAIL_FROM || "Dance for His Glory <onboarding@resend.dev>",
      to: [env.EMAIL_TO],
      subject,
      text,
    };
    // Lets you reply straight to the donor from your inbox.
